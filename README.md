...
local Toggle = getgenv().Gult.Key360.Toggle
local RotationSpeed = getgenv().Gult.Key360.RotationSpeed
local Keybind = getgenv().Gult.Key360.Keybind
...
local uwuMain = uwuGult.General
local uwuCamMain = uwuGult.Camlock.Main
local uwuCamFOV = uwuGult.Camlock.FOV
local uwuSilentMain = uwuGult.Silent.Main
local uwuSilentFOV = uwuGult.Silent.FOV
local uwuTrace = uwuGult.Tracer
local uwuAutoPred = uwuGult.AutoPrediction
...
local Notification = loadstring(game:HttpGet("https://pastebin.com/raw/BzPeJSGz"))()
...
if uwuMain.Notifications then
    SendNotification("report if problem")
    wait(3.5)
